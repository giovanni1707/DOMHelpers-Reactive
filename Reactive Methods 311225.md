...
- `ReactiveCleanup.patchState(state)` - Manually patch existing state 
- `ReactiveCleanup.isActive(effectFn)` - Check if effect is still active 
- `ReactiveCleanup.getStats()` - Get diagnostic information 
- `ReactiveCleanup.debug(enable)` - Enable or disable debug mode 
- `ReactiveCleanup.test()` - Run test to verify cleanup system 
...
- `withStorage(reactiveObj, key, options)` - Backward compatibility alias 
